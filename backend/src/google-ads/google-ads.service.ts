...
export class GoogleAdsService {
    ...
    try {
      ...
      if (retryCandidates.length) {
        ...
        const retryPrompt = [
          ...
          'Bản sửa vẫn phải giữ cùng thông điệp trung tâm với các HEADLINE/DESCRIPTION đã đạt. Tiêu đề nêu ý chính; mô tả phải bổ sung đúng ý đó, không được chuyển sang tính năng hoặc lợi ích khác.',
          'Trước khi trả về, ghép thử HEADLINE và DESCRIPTION của bộ nội dung sau khi sửa; nếu chúng không thể xuất hiện cùng một mẫu quảng cáo thì phải viết lại.',
          'Không cần dịch hoặc giữ ý của nội dung cũ nếu nội dung đó sai ngôn ngữ, vô nghĩa, quá ngắn hoặc không liên quan.',
          'Trong trường hợp đó, hãy sáng tạo một câu hoàn toàn mới dựa trên chủ đề nhóm, nội dung quảng cáo hiện có và ngôn ngữ bắt buộc.',
  ...
  private buildOpenAiTextSuggestionPrompt(
    ...
    const editableSystemPrompt = this.repairUtf8Mojibake(context.editableSystemPrompt?.trim() || guidance?.editableSystemPrompt?.trim() || [
      ...
      'Tiêu đề và mô tả phải là một cặp cùng thông điệp: tiêu đề nêu ý chính, mô tả bổ sung đúng lợi ích hoặc hành động của ý đó.',
      'Đối chiếu toàn bộ tiêu đề và mô tả trước khi trả kết quả; viết lại nếu chúng lệch ý hoặc không thể xuất hiện cùng một mẫu quảng cáo.',
    ].join('\n'));
    ...
    return [
      ...
      '6. Không lặp lại cùng thông điệp; các đề xuất phải đa dạng góc thuyết phục và liên quan đến chủ đề nhóm hoặc nội dung ứng dụng hiện có.',
      ...
      '15. BẮT BUỘC coi HEADLINE và DESCRIPTION là các phần của cùng một mẫu quảng cáo, không phải các câu độc lập. Chọn một thông điệp trung tâm dựa trên chủ đề nhóm; HEADLINE nêu ngắn gọn thông điệp đó và DESCRIPTION giải thích, củng cố lợi ích hoặc đưa ra hành động phù hợp cho chính thông điệp đó.',
      '16. Không được tạo HEADLINE nói về một tính năng/lợi ích nhưng DESCRIPTION lại nói sang tính năng/lợi ích khác. Mỗi DESCRIPTION mới phải ghép hợp lý với ít nhất một HEADLINE mới hoặc hiện có; mỗi HEADLINE mới phải ghép hợp lý với ít nhất một DESCRIPTION mới hoặc hiện có.',
      '17. Nếu trong cùng lần chạy có cả HEADLINE LOW và DESCRIPTION LOW, hãy viết chúng như một bộ nội dung đồng nhất. Trước khi trả JSON, đọc thử từng cặp HEADLINE + DESCRIPTION; nếu người đọc không thấy chúng cùng một ý hoặc cùng lời hứa quảng cáo thì phải viết lại.',
      '',
      ...
      'Tất cả nội dung thay thế trong cùng nhóm phải bám một thông điệp trung tâm thống nhất. Sự đa dạng chỉ nằm ở cách diễn đạt, không được làm tiêu đề và mô tả lệch chủ đề hoặc mâu thuẫn nhau.',
      ...
      'Nội dung LOW chỉ là vị trí cần thay, không phải yêu cầu phải dịch hoặc giữ nguyên ý. Nếu nội dung cũ sai ngôn ngữ, vô nghĩa hoặc không liên quan, hãy viết câu hoàn toàn mới từ chủ đề nhóm và các nội dung hợp lệ khác trong nhóm.',
      'Khi chủ đề nhóm và ngôn ngữ đã được cấu hình, phải trả về đề xuất cho mọi candidate; không được bỏ trống chỉ vì nội dung cũ kém chất lượng.',
